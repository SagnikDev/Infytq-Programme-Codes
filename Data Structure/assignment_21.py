...
#Global variables
flight_details=["AI345:BAN:CAN:1410","AI678:BAN:LON:1200","AI890:BAN:MUM:1400","AF780:BAN:AGF:1340","AI001:BAN:AUS:1500","AI404:BAN:NY:1220"]
...
def find_flights(flight_time):
    ...
    for flight in flight_details:
        flight_time_exact=flight.split(":")[3]
        if(int(flight_time_exact) in range(flight_time,flight_time+200)):
            flight_list.append(flight)
    return flight_list
def sort_flight_list(flight_list):
    flights={}
    for flight in flight_list:
        flights[int(flight.split(":")[3])]=flight
    flights=sorted(flights.values(),reverse=True)
    return (flights)

def get_passenger_details(flight_detail):
    pnr_list=[]
    flight=flight_detail.split(":")[0]
...
def security_check(passenger_pnr_list):
    ...
    for passenger in passenger_pnr_list:
        if(passenger_details_dict[passenger][3] in range(0,26)):
            pnr_list.append(passenger)
    return pnr_list

def sort_passengers(passenger_pnr_list):
    pnr_dict={}
    for passenger in passenger_pnr_list:
        pnr_dict[passenger_details_dict[passenger][2]]=passenger
    pnr_dict=sorted(pnr_dict.values())
    return (pnr_dict)

def boarding(passenger_pnr_list):  
    boarding=Queue(len(passenger_pnr_list))
    for passenger in passenger_pnr_list:
        boarding.enqueue(passenger)
    return boarding

def seating(passenger_queue):
    seat=Stack(passenger_queue.get_max_size())
    while(passenger_queue.is_empty()==False):
        seat.push(passenger_queue.dequeue())
    return seat
