...
class People:
    ...
    def check_gender(people_queue,gender):
        result_queue=Queue(people_queue.get_max_size())
        while(people_queue.is_empty()==False):
            people=people_queue.dequeue()
            if(people.__gender==gender):
                result_queue.enqueue(people)
        return result_queue
    def __str__(self):
