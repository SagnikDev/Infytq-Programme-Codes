...
class Item:
    ...
    def __str__(self):
        name=str(self.__item_name)+" "+str(self.__author_name)+" "+str(self.__published_year)
        return name
...
class Library:
    ...
    def sort_item_list_by_author(self,new_item_list):
        sorted_authors=[]
        sorted_item_list=[]
        for book in new_item_list:
            sorted_authors.append(book.get_author_name())
        sorted_authors.sort()
        for author in sorted_authors:
            for book in new_item_list:
                if(book.get_author_name()==author):
                    sorted_item_list.append(book)
    ...
    def sort_items_by_published_year(self):
        ...
        for book in self.__item_list:
            sorted_year.append(book.get_published_year())
        sorted_year.sort()
        for i in sorted_year:
            for book in self.__item_list:
                if(book.get_published_year()==i):
                    sorted_item_year.append(book)
                    self.__item_list.remove(book)
                    break
        self.__item_list= sorted_item_year
