...
def separate_boxes(box_stack):
    ...
    prime_colors=Stack(box_stack.get_max_size())
    while(box_stack.is_empty()==False):
        color=box_stack.pop()
        if(color in ["Red","Green","Blue"]):
            prime_colors.push(color)
        else:
            result.enqueue(color)
    while(prime_colors.is_empty()==False):
        color=prime_colors.pop()
        box_stack.push(color)
    return result
